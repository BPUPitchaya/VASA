...
from .api.endpoints import bp as api_bp  # blueprint with all /api routes

app = Flask(__name__)

# CORS – keep it simple and target /api/*
CORS(app, resources={r"/api/*": {"origins": "*"}})

# Register ONLY the blueprint (do not define /api routes here)
app.register_blueprint(api_bp)

# Optional: add permissive headers for all responses
@app.after_request
def after_request(response):
    response.headers.add('Access-Control-Allow-Origin', '*')
    response.headers.add('Access-Control-Allow-Headers', 'Content-Type,Authorization')
